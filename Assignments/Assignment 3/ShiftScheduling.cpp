...
Set<Shift> helper(const Set<Shift>& shifts, const Set<Shift>& sofar, int maxHours) {
    // no choice
    if (shifts.isEmpty())
        return sofar;

    // kick out current shift and added into sofar if choose
    Shift sft = shifts.first();
    Set<Shift> remaining = shifts - sft;
    Set<Shift> ifChoose = sofar + sft;

    // check if overlaps with previous choices
    bool isOverlap = false;
    for (auto t : sofar) {
        if (overlapsWith(sft, t)) {
            isOverlap = true;
            break;
        }
    }

    // current shift can be choosed
    int length = lengthOf(sft);
    if (length <= maxHours && !isOverlap) {
        // choose current shift
        auto choose = helper(remaining, ifChoose, maxHours - length);
        // not choose current shift
        auto notChoose = helper(remaining, sofar, maxHours);
        // calculate the sum of two decisions respectively
        int sumChoose = 0;
        int sumNotChoose = 0;
        for (auto t : choose) sumChoose += valueOf(t);
        for (auto t : notChoose) sumNotChoose += valueOf(t);
        return sumChoose > sumNotChoose ? choose : notChoose;
    }

    // otherwise
    return helper(remaining, sofar, maxHours);
}
...
Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    if (maxHours < 0)
        error("highestValueScheduleFor: maxHours is less than 0");
    if (maxHours == 0)
        return {};

    return helper(shifts, {}, maxHours);
}
