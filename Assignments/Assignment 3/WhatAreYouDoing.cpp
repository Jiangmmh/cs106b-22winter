...
void helper(Set<string> &s, const Vector<string> &tokens, int idx, string sentence) {
    if (idx >= tokens.size()) {
        s.add(sentence);
        return;
    }
    ...
    if (!isalpha(tokens[idx][0])) {
        helper(s, tokens, idx + 1, sentence + tokens[idx]);
    } else {
        helper(s, tokens, idx + 1, sentence + toLowerCase(tokens[idx]));
        helper(s, tokens, idx + 1, sentence + toUpperCase(tokens[idx]));
    }
}
