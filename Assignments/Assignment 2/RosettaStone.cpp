...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    if (kGramLength < 1)
        error("KgramsIn: kGramLength is less than 1");
    ...
    for(int i = 0; i <= n - kGramLength; i++) {
        string currentGram = str.substr(i, kGramLength);
        mp[currentGram] += 1.0;
    }
...
Map<string, double> normalize(const Map<string, double>& input) {
    if (input.isEmpty())
        error("normalize: input is empty");

    int countNonZero = 0;
    double sumSquare = 0.0;
    for (const string &gram : input.keys()) {
        double t = input[gram];
        if (t != 0.0) {
            countNonZero++;
            sumSquare += pow(t, 2);
    ...
    if (countNonZero == 0)
        error("normalize: there are all zeros value in map");

    double sumSquareRoot = sqrt(sumSquare);
    Map<string, double> result;
    for (const string &key : input.keys()) {
        result[key] = input[key] / sumSquareRoot;
    }
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    if (numToKeep < 0)
        error("topKGramsIn: numtoKeep is less than zero");
    ...
    // push all the elements into prioqueue
    PriorityQueue<string> pq;
    for (const string &gram : source.keys()) {
        pq.enqueue(gram, source[gram]);
    }

    // dequeque n - numToKeep element from prioqueue
    for (int i = 0; i < n - numToKeep; i++) {
        pq.dequeue();
    }

    // put top numToKeep pair into result map
    Map<string, double> result;
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    if (corpora.isEmpty())
        error("guessLanguageOf: corpora is empty");

    double maxSimilarity = 0.0;
    string mostSimilarLang;

    for (auto cop : corpora) {
        double similarity;
        similarity = cosineSimilarityOf(textProfile, cop.profile);
        if (similarity > maxSimilarity) {
            maxSimilarity = similarity;
            mostSimilarLang = cop.name;
        }
    }

    return mostSimilarLang;
}
