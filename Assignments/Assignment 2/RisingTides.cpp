...
GridLocation getNeibor(const GridLocation &pos, int sel) {
    switch (sel) {
    case 0: return {pos.row, pos.col - 1};
    case 1: return {pos.row, pos.col + 1};
    case 2: return {pos.row - 1, pos.col};
    case 3: return {pos.row + 1, pos.col};
    default:
        return {};
    }
}
...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    // push water souce into queue
    for (auto elem : sources) {
        if (terrain[elem.row][elem.col] <= height) {
            q.enqueue(elem);
            ans.set(elem.row, elem.col, true);
        }
    }

    while (!q.isEmpty()) {
        auto curPos = q.dequeue();
        for (int i = 0; i < 4; i++) {
            auto nextPos = getNeibor(curPos, i);
            if (terrain.inBounds(nextPos.row, nextPos.col)
                && !(ans.get(nextPos.row, nextPos.col))
                && terrain.get(nextPos.row, nextPos.col) <= height) {
                q.enqueue(nextPos);
                ans.set(nextPos.row, nextPos.col, true);
            }
        }
    }
