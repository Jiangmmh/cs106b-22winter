...
string randomShuffle(string input) {
    if (input == "")
        return "";
    int randIndex = randomInteger(0, input.size() - 1);
    char pick = input[randIndex];
    input.erase(input.begin() + randIndex);
    return pick + randomShuffle(input);
}
...
STUDENT_TEST("Provided Test: Verify that solution is at least valid permutation.") {
    string res = randomShuffle("");
    sort(res.begin(), res.end());
    EXPECT_EQUAL(res, "");
}
